...
def random_card():
    """randomcard API, returns information about a random card chosen from a list generated by filters
    """
    ...
    game = Games[game_key]
    card = game.random_card(player_name, filter)
    if not card:
...
def card_action():
    ...
    target_player: a string containing a target player's name, defaults to an empty string
    ...
    if game_key not in Games:
        return make_response('Game Not found', INVALID_SESSION_ERROR)
    game = Games[game_key]
    game.card_action(player_name, target_player, cardid, action)
    print("card action:{} {} {}".format(action, cardid, player_name))
...
def query_players():
    ...
    game_key = request.form.get('game_key', session['game_key'])
    exclude_self = request.form.get('exclude_self')
    exclude_player = ''
    if exclude_self:
        exclude_player = session['player_name']
    if game_key not in Games:
        return make_response('Game Not found', INVALID_SESSION_ERROR)
    game = Games[game_key]
    # create and return single-element dictionary for Json format
    return {'players': [pname for pname in game.players.keys() if pname != exclude_player]}
